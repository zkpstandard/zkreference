...
in comparison with the baseline version 0.1, in order to achieve version 0.2.
	Each table, indexed as \letterIndexPropContrib$x$ (where $x$ is an integer), 
corresponds to a \href{https://github.com/zkpstandard/zkreference/issues}{GitHub issue} 
(GI$y$, where $y$ is an integer) describing proposed contributions --- 
see \myurl{https://github.com/zkpstandard/zkreference/issues}.
	However, compared with GitHub, the description here may have been adjusted for 
a better explanation and cross-referencing of the actual edits made in the document.
...
\item \textbf{Item id:} 
	An index (e.g., \ref{it:editorial:add-abstract}) of the contribution item, with a numbering 
subordinate to index (e.g., \hyperref{issue:editorial-structural}) the table where it belongs.
