...
\notbool{boolShowFeedback}{
		%%%% IF NOT INSERTING THE TABLE OF COMMENTS
		\renewcommand{\revblock}[2][]{} %then also do not tag the revision blocks in the main text
	}{
		%%% IF DECIDING TO INSERT THE TABLE OF COMMENTS
		\cleartooddpage %%%\clearpage
	...
	\fancypagestyle{lscapetablecontribs}{
		...
		\vspace{1em}
		The following pages describe proposed contributions to upgrade the draft reference document from version 0.1 
	(dated 2019-04-11, available during the 2nd ZKProof Workshop) to version 0.2.
		\red{[[The current draft 0.1.x only contains description of proposed contributions for 
		which, at the time of writing, there are proposed contributors.
			The draft version 0.2 (to appear) will only mention the actual committed contributions;
		other contributions may be deferred to a future version.]]}
...
made to the baseline version 0.1, in order to achieve version 0.2.
	...
	%\phantomsection\pdfbookmark[1]{Contributions}{pdfbkm:feedback-index}
	\input{zzz-01-comments-on-v0.1.tex}
}
