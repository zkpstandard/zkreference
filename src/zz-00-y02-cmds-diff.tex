...
\renewcommand{\pdfcomment}[2][]{}  %%% Disabling popup annotations, rendering ineffective some commands below
...
\providecommandtwoopt{\revblock}[3][][0pt]{\marginnote{\hspace*{#2}\resizebox{.85in}{!}{{\normalsize\normalfont\color[rgb]{1,0,0}%
			...
			\end{minipage}}}}}
\providecommandtwoopt{\revblockmini}[3][][]{{\scriptsize\revblock[#1][#2]{#3}}}
...
\def\colNamePropContrib{Contribution \textbf{d}escription}
\def\letterIndexPropContrib{D}
...
\newcommand\listcontributionname{List of Contribution Descriptions}
